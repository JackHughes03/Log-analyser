...
def get_response_codes(
    ...
    sorted_codes = sorted(
        response_code_dictionary.items(), key=lambda x: x[1], reverse=True
    )
    processed = 0
    ...
    for response_code, count in sorted_codes[:30]:
        ...
        color = "#ffffff"
        set_text_callback(
            text_label, f"{response_code} - appears {count} times.", color, scroll_area
        )
...
def get_country_and_isp(ip):
    try:
        ...
        response = requests.get(
            f"http://ipinfo.io/{ip}?token=4ee37a1361c69b", timeout=5
        )
